...
func (p *Proxy) handshake() error {
	...
	if !hasPassAuth {
		...
		logrus.Errorln(p.id, p.srcAddr(), "no supported auth method")
...
func (p *Proxy) passwordAuth() error {
	...
	password := buf[p3:p4]
	if p.auth.Verify(user, string(password), p.conn.RemoteAddr().String()) {
...
func (p *Proxy) processRequest(tcpIn chan<- *constant.TCPContext) error {
	...
	case CmdConnect, CmdBind:
		return p.handleConnectCmd(target, tcpIn)
	default:
		return errors.New("command not supported")
	}
}

func (p *Proxy) handleConnectCmd(target string, tcpIn chan<- *constant.TCPContext) error {
